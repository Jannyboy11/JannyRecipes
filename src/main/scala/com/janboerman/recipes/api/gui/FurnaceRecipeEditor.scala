...
import com.janboerman.recipes.api.recipe.{FurnaceRecipe, SimpleFurnaceRecipe}
...
import com.janboerman.recipes.api.JannyRecipesAPI
import com.janboerman.recipes.api.recipe.{FurnaceIngredient, FurnaceRecipe, SimpleFurnaceRecipe}
...
class FurnaceRecipeEditor[P <: Plugin](inventory: Inventory,
    ...
    with RecipeEditorModifiersOps[FurnaceRecipe] { self =>
    ...
    override def makeRecipe(): Option[FurnaceRecipe] = {
        ...
        if (ingredientStack != null) {
            ...
            val recipe: FurnaceRecipe = new SimpleFurnaceRecipe(key, group, ingredient, result, cookingTime, experience)

            //apply modifiers TODO use higher kinded types? type classes? structural types? use Shapeless?
            //TODO should instead our return type be Option[Recipe] ?
            for (modifier <- getModifiers()) {
//                recipe = modifier.apply(recipe)
            }
